...
import com.cookMeGood.makeItTasteIt.container.LogContainer.INFO_SAVE_CONTENT_DELETE
import com.cookMeGood.makeItTasteIt.container.LogContainer.INFO_SAVE_CONTENT_SAVE
import java.io.File
...
object SaveContentHelper {
    ...
    fun saveRecipeImageToInternalStorage(
        ...
        try {
            ...
            Log.i(INFO_SAVE_CONTENT_SAVE, "Recipe image was saved successfully.")
        } catch (e: IOException) {
            e.printStackTrace()
            Log.i(INFO_SAVE_CONTENT_SAVE, "Recipe image saving failed!")
        }
    ...
    fun deleteRecipeImageFromInternalStorage(applicationContext: Context, filename: String): Boolean {
        return if (applicationContext.deleteFile(filename)) {
            Log.i(INFO_SAVE_CONTENT_DELETE, "Recipe image was successfully deleted.")
            true
        }
        else {
            Log.i(INFO_SAVE_CONTENT_DELETE, "Recipe image deletion failed!")
            false
        }
